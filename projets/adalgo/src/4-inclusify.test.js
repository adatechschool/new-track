...
test("il/elle are properly replaced", () => {
    expect(inclusify("Aujourd'hui elle a push le code sur github")).toBe("Aujourd'hui elle a push le code sur github");
    expect(inclusify("Il a bu un café")).toBe("Iel a bu un café");
    expect(inclusify("Il a mangé tout les chocolats et elles sont parti en racheter.")).toBe(
        "Iel a mangé tout les chocolats et iels sont parties en racheter."
    );
    ...
    expect(inclusify("Elle est allé sur l'ile.")).toBe("Iel est allé sur l'ile.");
});
